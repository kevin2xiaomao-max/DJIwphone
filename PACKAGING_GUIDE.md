...
- 网络可访问 GitHub，以获取 `WebRTC-lib` 151.0.1 pod
...
之后始终打开 **`DJIwphone.xcworkspace`**，不要打开未集成 Pods 的 `.xcodeproj`。选择唯一 scheme：`DJIwphone`。
...
```bash
xcodebuild test -workspace DJIwphone.xcworkspace -scheme DJIwphone \
...
```bash
xcodebuild build -workspace DJIwphone.xcworkspace -scheme DJIwphone \
...
- `pod` 找不到：安装 CocoaPods 后重新执行 `pod install --repo-update`。
- `No such module WebRTC`：不要手动导入 Swift WebRTC 模块；本工程通过 CocoaPods 集成其二进制依赖，清理后重新 `pod install`。
- 找不到 Simulator：先执行 `xcrun simctl list devices available`，使用实际存在的 UDID/名称。
- AppIcon/Assets.car 缺失：确认打开的是 workspace，并重新运行 `xcodegen generate` 后构建。
...
3. 依赖：`brew install xcodegen && sudo gem install cocoapods && xcodegen generate && pod install --repo-update`
4. 打包：打开 `DJIwphone.xcworkspace`，scheme 选 `DJIwphone`，执行 Device Release 命令并按上文生成 IPA
